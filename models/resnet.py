...
# https://github.com/yaoxufeng/PCL-Proxy-based-Contrastive-Learning-for-Domain-Generalization/blob/86b1d1bdddb2e93f234faff128d805a2f1931d7c/domainbed/networks.py
...
class PCLResNet(nn.Module):
    def __init__(self, network, proj_dim=512, n_outputs=2048):
        super(PCLResNet, self).__init__()
        self.network = network
        self.network.fc = Identity()
        self.fea_proj = nn.Sequential(
            nn.Linear(proj_dim, proj_dim),
        )
        self.fc_proj = nn.Parameter(torch.FloatTensor(proj_dim, proj_dim))

        dropout = nn.Dropout(0.25)
        self.encoder = nn.Sequential(
            nn.Linear(n_outputs, proj_dim),
        ...
        self.classifier = nn.Parameter(torch.FloatTensor(network.num_classes, proj_dim))
...
class ResNet(nn.Module):
    def __init__(
        ...
        if projection_head:
            ...
            self.bn_last = nn.BatchNorm1d(512 * width_multiplier * block.expansion)
            self.fc2 = nn.Linear(
                512 * width_multiplier * block.expansion,
                512 * width_multiplier * block.expansion,
            )
            self.fc_final = nn.Linear(
                512 * width_multiplier * block.expansion, self.num_classes
            )
        else:
            self.fc = nn.Linear(
                512 * width_multiplier * block.expansion, self.num_classes
